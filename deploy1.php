...
session_start();
    ...
    if(!isset($_SESSION['sess_username']) || $role!="Release Team" ){
...
$query1=mysqli_query($con,"SELECT spath from vtable where vername='$vername'");
...
$spath=$query2['spath'];
    $final_path=$spath."/".$vername."//";
    mysqli_query($con,"DELETE FROM vtable WHERE vername='$vername' "); //DELETE THE ENTRY OF FINAL BUILD
    $source=$final_path;
    $dest="c:/FINAL RELEASES/".$vername;
...
width:500px;
  ...
  <a href="release.php">Dashboard</a>
...
echo "<select name='vername' class='cs-select cs-skin-rotate'>";
 echo "<option value='" . "CHOOOSE" ."'>" . "CHOOOSE" ."</option>";
...
echo "</select>";
			...
			<div class="txt">
							<div class="li">
				<input class="lu" name="submit" type="submit" id="pa" onclick="myFunction()" value="Release" style="padding-left:87px;padding-right:87px"/></div>
				<!--<div class="li">
				<a href="status.php" class="lu">Status</a>
			</div>--> 
			</div>
...
<script>
    function myFunction() {
	    ...
	    swal({   title: "Done!",   text: " Released successfully",   timer: 2000,   showConfirmButton: false });
    }
	...
	while($it1->valid()) 
	{
        ...
        var barChartData = {
            ...
            datasets: [ {
                ...
                borderWidth: 1,
                data: [ '<?php echo $i1 ;?>','<?php echo $i2;?>','<?php echo $i3;?>','<?php echo $i4 ;?>','<?php echo $i5 ;?>']
            }]
